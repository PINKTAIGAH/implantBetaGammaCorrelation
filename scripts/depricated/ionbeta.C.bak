...
namespace constants {
    ...
    const int64_t TIME_SCALE = 1e6; // Timescale of time wrt ns
    const int64_t TIME_THRESHOLD = 500 * TIME_SCALE; // 1000 ms
    const int64_t POSITION_THRESHOLD = 1; // 1 strips in X and Y around the centre

    const double HALF_LIFE = 50;
    const double HALF_LIFE_NS = HALF_LIFE * TIME_SCALE;

    const std::string ISOTOPE_TREE = "82nb";

    const int LIFETIME_BINS = 500;
}
...
void ionbeta(const char* input, const char* output){
    ...
    //************* DEGUB **************
    /*std::cout << "aida_gatedimplant_"+constants::ISOTOPE_TREE+"_tree" << std::endl;*/
    /*std::cout << "gatedimplant_"+constants::ISOTOPE_TREE+".e" << std::endl;*/
    /*std::exit(0);*/
    //************* DEGUB **************
    ...
    int neighbouring_position_bins = constants::POSITION_THRESHOLD*2 + 1;
    ...
    TH1F* aida_implant_veto_dt = new TH1F("aida_implant_veto_dt", "Implant-Decay #Deltat;Implant-Decay #Deltat (); Counts/", constants::LIFETIME_BINS, -constants::TIME_THRESHOLD/constants::TIME_SCALE, constants::TIME_THRESHOLD/constants::TIME_SCALE);

    TH1F* implantbeta_candidate_multiplicity = new TH1F("implantbeta_candidate_multiplicity", "Implant-Decay Candidate Multiplicity; Candidate Multiplicity; Counts", 100, 0, 100);
    TH2F* implantbeta_candidate_hitpattern = new TH2F("implantbeta_candidate_hitpattern", "Implant-Decay Canditate; X Strip; Y Strip", neighbouring_position_bins, -constants::POSITION_THRESHOLD,constants::POSITION_THRESHOLD, neighbouring_position_bins, -constants::POSITION_THRESHOLD, constants::POSITION_THRESHOLD);
    TH1F* implantbeta_candidate_hitpattern_x = new TH1F("implantbeta_candidate_hitpattern_x", "Implant-Decay Canditate X Hit Patterns; Counts; X Strip", neighbouring_position_bins, -constants::POSITION_THRESHOLD, constants::POSITION_THRESHOLD);
    TH1F* implantbeta_candidate_hitpattern_y = new TH1F("implantbeta_candidate_hitpattern_y", "Implant-Decay Canditate Y Hit Patterns; Counts; Y Strip", neighbouring_position_bins, -constants::POSITION_THRESHOLD, constants::POSITION_THRESHOLD);
    ...
    int matched_backwards_implantdecays = 0;
    ...
    for (auto it = gated_implants.begin(); it != gated_implants.end(); it++) {
        ...
        if (type == IMPLANT && dssd == 1) {
            ...
            int implantbeta_candidate_counter = 0; // Set counter to check all beta candidates
            ...
            auto decay_start = good_decays.lower_bound(last_gatedimplant_time - constants::TIME_THRESHOLD);
            for(auto decay_it = decay_start; decay_it != good_decays.end(); decay_it++){
                ...
                if (decay_type == DECAY && TMath::Abs(decay_x - gatedimplant_pos_x) <= constants::POSITION_THRESHOLD && TMath::Abs(decay_y - gatedimplant_pos_y) <= constants::POSITION_THRESHOLD){ {
                        ...
                        if (time_diff > decay_it->first + last_gatedimplant_time){
                            break;
                        }
                        
                        if (0 < time_diff && time_diff < constants::TIME_THRESHOLD) {
                            ...
                            if (!found_forward_candidate){
                                ...
                                found_forward_candidate = true;
                                //************* DEGUB **************
                                std::cout << decay_x << " " << decay_y << " " << time_diff/constants::TIME_SCALE  << std::endl;
                                //************* DEGUB **************
                            }
                            implantbeta_candidate_counter++;
                        }
                        
                        if (!constants::CHECK_BETA_CANDITATES && !constants::INCLUDE_BACKWARDS_MATCH && found_forward_candidate){
                            break;
                        }
          
                        if (constants::INCLUDE_BACKWARDS_MATCH && time_diff < 0 && -time_diff < constants::TIME_THRESHOLD) {
                            if (!found_backwards_candidate){
                                aida_implant_veto_dt->Fill(time_diff/constants::TIME_SCALE);
                                matched_backwards_implantdecays++;
                                found_backwards_candidate = true;
                                //************* DEGUB **************
                                std::cout << decay_x << " " << decay_y << " " << time_diff/constants::TIME_SCALE  << std::endl;
                                //************* DEGUB **************
                            }
                        ...
                        // !!!!!!!!!!! CHANGE THIS PLEASE !!!!!!!!!!!!!!!!!!!
                        if (!constants::CHECK_BETA_CANDITATES && constants::INCLUDE_BACKWARDS_MATCH && found_backwards_candidate && found_forward_candidate){
                            break;
                        }
          // !!!!!!!!!!! CHANGE THIS PLEASE !!!!!!!!!!!!!!!!!!!
                        /*else{*/
                    ...
                    implantbeta_candidate_hitpattern->Fill(decay_x - gatedimplant_pos_x, decay_y - gatedimplant_pos_y);
                    implantbeta_candidate_hitpattern_x->Fill(decay_x - gatedimplant_pos_x);
                    implantbeta_candidate_hitpattern_y->Fill(decay_y - gatedimplant_pos_y);
                }
            }
            implantbeta_candidate_multiplicity->Fill(implantbeta_candidate_counter); 
        }
    }
    ...
    std::cout << "Matched: " << matched_backwards_implantdecays << " backwards gated implant events" << std::endl;
    ...
    if (constants::CHECK_BETA_CANDITATES){ implantbeta_candidate_multiplicity->Write(); }
    if (constants::CHECK_BETA_CANDITATES){ implantbeta_candidate_hitpattern->Write(); }
    if (constants::CHECK_BETA_CANDITATES){ implantbeta_candidate_hitpattern_x->Write(); }
    if (constants::CHECK_BETA_CANDITATES){ implantbeta_candidate_hitpattern_y->Write(); }

...
void makeAnatrees(const char* input, const char* output) {
    ...
    decay_tree->Branch("decay", &aida_decay_data, "time/l:x/D:y/D:e/D:ex/D:ey/D:sp/I");
...
// Implants in coincidence with FRS
    ...
    if(aidadecayhits == 1){
        for (int i = 0; i < aidadecayhits; i++) {
            if (decay_dssd[i] == 1 && TMath::Abs(decay_time_x[i] - decay_time_y[i]) < 5e3 && TMath::Abs(decay_energy_x[i] - decay_energy_y[i]) < 168 && decay_energy[i] > 151 && decay_energy[i] < 3000 && TMath::Abs(decay_time[aidadecayhits -1] - decay_time[0]) < 33000) {
...
//             for (int i = 0; i < aidadecayhits; i++) {
//                 if (decay_dssd[i] == 1 /* && TMath::Abs(decay_time_x[i] - decay_time_y[i]) < 3.e3 && TMath::Abs(decay_energy_x[i] - decay_energy_y[i]) < 150 */) {
...
//                                 // if(filled_aidadecaytree.count(i) == 0 /* && germanium_energy[j] > 1035 && germanium_energy[j] < 1045 */){
//                                 //     aida_decay_data.time = decay_time[i];
//                                 //     aida_decay_data.x = decay_x[i];
//                                 //     aida_decay_data.y = decay_y[i];
//                                 //     aida_decay_data.energy = decay_energy[i];
